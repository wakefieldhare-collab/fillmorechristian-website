...
if (Test-Path -LiteralPath $calendarPath) {
    ...
    if ($indexHtml -notmatch 'data-schedule-exception="2026-10-04"' -or
        ...
        $eventsHtml -notmatch 'Community Church Service at Fillmore Ballpark') {
        $calendarIssues.Add("home or events page is missing the October 4 community service notice")
    }
...
if ((Test-Path -LiteralPath $deployScriptPath) -and (Test-Path -LiteralPath $pagesAuthScriptPath)) {
    ...
    if ($deployScriptText -match "wake-byte" -and
        ...
        $deployScriptText -match "ls-remote origin" -and
        $deployScriptText -match "AllowUnpushed" -and
        $deployScriptText -match "not pushed to origin" -and
        ...
        Add-Check "Cloudflare deploy script" "OK" "Guarded Pages deploy script checks owner, pushed personal main, Pages-token or Wrangler OAuth project auth, local preflight, and Wrangler deploy"
    } else {
        Add-Check "Cloudflare deploy script" "FAIL" "Deploy script is missing owner guard, pushed-main guard, Pages-token preflight, local preflight, npm auth alias, or Wrangler deploy command"
    }
} else {
...
$dnsCacheStatusScriptPath = Join-Path $root "scripts\show-dns-cache-status.ps1"
$registrarChecklistScriptPath = Join-Path $root "scripts\show-registrar-cutover-checklist.ps1"
$packageJsonPath = Join-Path $root "package.json"
if ((Test-Path -LiteralPath $statusScriptPath) -and (Test-Path -LiteralPath $dnsCacheStatusScriptPath) -and (Test-Path -LiteralPath $registrarChecklistScriptPath) -and (Test-Path -LiteralPath $packageJsonPath)) {
    $statusScriptText = Get-Content -Raw -LiteralPath $statusScriptPath
    $dnsCacheStatusScriptText = Get-Content -Raw -LiteralPath $dnsCacheStatusScriptPath
    ...
    if ($statusScriptText -match "npx wrangler login" -and
        ...
        $dnsCacheStatusScriptText -match "Longest observed stale TTL" -and
    ...
    Add-Check "Migration status command" "FAIL" "scripts\show-migration-status.ps1, scripts\show-dns-cache-status.ps1, scripts\show-registrar-cutover-checklist.ps1, or package.json is missing"
}
...
if ($feeds.ContainsKey($feedPaths[0]) -and (Test-Path -LiteralPath $homePath)) {
    ...
    $latestTitlePattern = if ($latestTitle) {
        "(?:$([regex]::Escape($latestTitle))|$([regex]::Escape([System.Net.WebUtility]::HtmlEncode($latestTitle))))"
    } else {
        ""
    }
    ...
    if ($latestTitle -and
        ...
        $homeHtml -match $latestTitlePattern -and
...
if (Test-Path -LiteralPath $podcastScriptPath) {
    ...
    if ($podcastScript -match "podcast-latest-list" -and
        ...
        (-not $latestTitle -or $podcastPageText -match $latestTitlePattern)) {
